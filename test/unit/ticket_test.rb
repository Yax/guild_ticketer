...
class TicketTest < ActiveSupport::TestCase
  ...
  should_belong_to :ticket_category
  should_have_many :messages

  should_validate_presence_of :email, :employee_name, :basic_state
  should_validate_presence_of :ticket_category_id, :message => "is not a valid ticket_category"
  ...
  should "have valid ticket_category" do
    wrong_ticket = tickets(:bledny_ticket)
    assert !wrong_ticket.valid?
    assert wrong_ticket.errors.invalid?(:ticket_category_id)
  end
